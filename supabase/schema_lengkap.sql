    ...
    BEGIN
        ...
        ON CONFLICT (id) DO UPDATE SET
            email = COALESCE(EXCLUDED.email, public.profil_pengguna.email),
    ...
    -- 11. Perbaikan Otomatis Token auth.users & Konfirmasi Email (Mencegah "Database error querying schema")
    UPDATE auth.users
    SET 
    ...
    -- Pastikan semua user memiliki identitas di auth.identities untuk login email
    DO $$
    BEGIN
        IF EXISTS (
    ...
    -- Sinkronkan kolom email pada profil_pengguna & data awal dari auth.users
    ALTER TABLE public.profil_pengguna ADD COLUMN IF NOT EXISTS email TEXT;
    UPDATE public.profil_pengguna p
    SET email = u.email
    FROM auth.users u
    ...
    -- 12. Fungsi RPC: Pendaftaran Cepat Bebas Email Rate Limit (Aman dari Schema Error)
    ...
    BEGIN
        IF EXISTS (SELECT 1 FROM auth.users WHERE email = lower(p_email)) THEN
            -- Sinkronkan token agar valid untuk login
            UPDATE auth.users
            SET 
                confirmation_token = COALESCE(confirmation_token, ''),
                recovery_token = COALESCE(recovery_token, ''),
    ...
    -- 13. Fungsi RPC: Cek Apakah Email Sudah Terdaftar di Database (Akurat untuk Validasi Halaman Login)
    CREATE OR REPLACE FUNCTION public.cek_email_terdaftar(p_email TEXT)
    RETURNS BOOLEAN
    LANGUAGE plpgsql
    SECURITY DEFINER
    ...
    GRANT EXECUTE ON FUNCTION public.cek_email_terdaftar(TEXT) TO anon, authenticated;
